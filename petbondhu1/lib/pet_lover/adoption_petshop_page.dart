...
class _AdoptionPetShopPageState extends State<AdoptionPetShopPage> {
  ...
  /// ✅ Add Adoption Post to Firestore
    ...
    if (_petNameCtrl.text.isNotEmpty && _petContactCtrl.text.isNotEmpty) {
      try {
        await FirebaseFirestore.instance.collection('post').add({
          ...
          'image': _petImageCtrl.text.trim(),
          'contact': _petContactCtrl.text.trim(),
          'timestamp': FieldValue.serverTimestamp(),
        });

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Adoption post added!")),
        );
  ...
  /// ✅ Shop still local (not Firestore yet)
  ...
  void _addShopProduct() {
    if (_shopOwnerCtrl.text.isNotEmpty &&
        _shopProductCtrl.text.isNotEmpty &&
        _shopContactCtrl.text.isNotEmpty) {
      setState(() {
  ...
  /// ✅ Adoption section with Firestore Stream
  Widget _buildAdoptionSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Post for Pet Adoption',
          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                return const Center(child: Text("No pets available yet."));
              }
                    ...
                    child: ListTile(
                      leading: imageUrl.isNotEmpty
                          ? Image.network(
                              ...
                              height: 56,
                              fit: BoxFit.cover,
                              errorBuilder: (_, __, ___) => const Icon(Icons.pets, color: Colors.teal),
                            )
  ...
  /// Shop section (local only)
  Widget _buildShopSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('List Pet Shop Products',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        const SizedBox(height: 8),
        TextField(
